...
func TestGetLoginURL(t *testing.T) {
	...
	req := test.NewRequest(
		...
		test.WithURL("http://localhost/login/myProvider"),
	)
...
func TestExchangeCode(t *testing.T) {
	...
	require.NoError(t, err)
	token := test.Base64Encoded(t, &oidc.JSONWebToken{ExpiresIn: sessionExpiresIn, Provider: "myProvider"})
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			req := test.NewRequest(
				test.WithMethod(http.MethodGet),
				test.WithURL(fmt.Sprintf("http://localhost/login/callback?code=myAuthCode&state=%s", rawState)),
...
func TestGetCurrentUser(t *testing.T) {
	...
	srv := loginServer(svc, staticTimeSvc)
	token := test.Base64Encoded(t, &oidc.JSONWebToken{})
	req := test.NewRequest(
		test.WithMethod(http.MethodGet),
	...
	assert.Equal(t, &web.ClientUser{Username: "myUser", Initials: "my"}, test.FromJSON[web.ClientUser](t, resp))
}
...
func TestLogout(t *testing.T) {
	...
	srv := loginServer(svc, staticTimeSvc)
	token := test.Base64Encoded(t, &oidc.JSONWebToken{Provider: "myProvider"})
	req := test.NewRequest(
		test.WithMethod(http.MethodGet),
...
func TestLogoutError(t *testing.T) {
	cases := []struct {
		...
		},
		{
			name: "Failed logout",
			sessionCookieValueFn: func(t *testing.T) string {
				return test.Base64Encoded(t, &oidc.JSONWebToken{Provider: "myProvider"})
			},
			statusCode: http.StatusInternalServerError,
			src: &mockOIDCService{
				FakeLogout: func(token *oidc.JSONWebToken) error {
...
func loginServer(svc oidc.Service, timeSvc clock.TimeService) *web.Server {
	...
	srv.RegisterRoutes(func(r fiber.Router) {
		login.Routes(r.Group("/"), cfgOidc, svc, timeSvc)
	})
